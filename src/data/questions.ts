...
export const questions: Question[] = [
  ...
  },
  {
    ...
    choices: [
      "To guarantee seed uniqueness across programs without including program id",
      "To search for a valid off-curve address for the seed tuple",
      "To let the runtime skip PDA derivation checks during invoke_signed",
      "To bind PDA signatures to a recent blockhash",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "A single global PDA that stores all users in one account",
      "A PDA per user or per position",
      "A PDA keyed by current slot so writes rotate every block",
      "One PDA per instruction type reused across users",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "A callee may mark an account as signer if it owns that account",
      "CPI can forward only equal or lower signer and writable privileges",
      "A callee may upgrade read-only to writable for executable accounts",
      "Inner instructions may restore signer bits dropped by outer calls",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "System Program plus native loader support with a writable programdata account",
      "The BPF Loader Upgradeable program",
      "Legacy BPF Loader with authority recorded in rent sysvar",
      "Feature gates that hot-swap program bytes without loader checks",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "To guarantee deployed bytecode matches local source files",
      "To ensure typed instruction encoding and decoding with stable layouts",
      "To auto-negotiate compute budget and priority fees for each method",
      "To remove the need to pass explicit account metas by fetching them at runtime",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "A single state account that stores all user records in one map",
      "Sharded state with one account per user or per market key",
      "Two shared state accounts split by read path and write path",
      "An epoch-rotating state account reused by all users",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "Use optimistic reads and resolve conflicts only at commit time",
      "Use shared read locks, so concurrent readers can access the same account",
      "Escalate to writable whenever any inner CPI marks the account mutable",
      "Bypass lock tracking and rely on replay-time version checks",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "To perform CPI without passing account metas to the callee",
      "To access runtime-provided functions such as hashing and sysvar reads",
      "To request heap or stack memory beyond configured program limits",
      "To perform validator-local I/O like network or filesystem access",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    tags: ["fees", "payer"],
    difficulty: "foundation",
    prompt: "In a transaction with multiple signers, where are base fees charged?",
    choices: [
      "From the first signer marked writable in the message header",
      "From the fee payer specified in the message header",
      "Split proportionally across all signers in the transaction",
      "From the leader vote account and later settled from rewards",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "To support multiple fee payers within a single message",
      "To enable Address Lookup Tables and larger account lists",
      "To move recent blockhash into a separate signed metadata account",
      "To allow CPI to append accounts dynamically during execution",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "By storing full account metas on-chain and referencing meta indices",
      "By storing addresses on-chain and referencing them by index",
      "By delta-encoding account keys against the fee payer key",
      "By reusing validator-local key caches keyed by transaction signature",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "Legacy and v0 messages if lookup tables are writable accounts",
      "v0 (versioned) transactions",
      "Only vote transactions resolved by validator replay",
      "Legacy messages when lookup tables are marked as sysvars",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "A PoH tick interval used to derive leader identity per hash",
      "A scheduled leader window in which a leader produces entries and blocks",
      "A lockout depth unit recorded in vote accounts for finality",
      "A rooted ledger height that cannot be rolled back",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "A finality mechanism that commits forks after supermajority votes",
      "A verifiable ordering and clock primitive used to sequence events efficiently",
      "A slashing rule that penalizes equivocation across forks",
      "A randomness source that selects leaders within each slot",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "They track delegation balances and lockups for stake accounts",
      "They cast votes on forks and earn rewards",
      "They distribute leader schedule updates to gossip peers each slot",
      "They store upgradeable program bytes for validator software",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      ...
      "Recent TPU throughput per validator",
  ...
  },
  {
    ...
    choices: [
      "Ingesting client transactions into the leader pipeline",
      "Verifying and replaying blocks to update state",
      "Broadcasting shreds and handling repair traffic",
      "Computing leader schedule and stake weights each epoch",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "Distribute shreds and repair packets between validators",
      "Propagate cluster metadata such as peers, votes, and contact info",
      "Forward end-user transactions into the current leader TPU",
      "Replicate full account-state snapshots continuously",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "Forward client transactions into leader ingest with QUIC flow control",
      "Efficiently broadcast blocks via shreds",
      "Propagate cluster metadata like peers and votes",
      "Replay executed blocks to update Bank state",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "Choose an exact slot hash to read independent of commitment semantics",
      "Select how finalized the queried data should be",
      "Request additional account fields in RPC responses",
      "Ask RPC nodes to retry until finalized before returning",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "When fixed-interval snapshots matter more than push latency",
      "When you need near-real-time account updates",
      "When you need long historical backfills from archived slots",
      "When you need a single read pinned to one slot",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "Always pinned to the node's highest root regardless of request options",
      "Dependent on the commitment level of the request",
      "Equivalent to finalized state after one cluster confirmation",
      "Fork-independent because RPC nodes reconcile state before responding",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "When validator code containing it ships in a stable release",
      "When most stake upgrades binaries that include the feature code",
      "When the feature gate is activated on-chain",
      "When testnet activation is complete and a mainnet epoch is scheduled",
    ],
    answerIndex: 2,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "To tie protocol behavior directly to validator software version numbers",
      "To activate changes safely and sequentially",
      "To remove coordinated activation transactions for runtime changes",
      "To bypass epoch boundaries when rolling out runtime logic",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "Logs are persisted in ledger entries, so they can replace account reads for state proofs",
      "Logs are hints for debugging and UX, not canonical on-chain state",
      "Logs are generated from post-state diffs, so they are canonical outputs",
      "Logs are written into a per-program sysvar that clients can verify on-chain",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "The runtime sorts instructions by program id before execution",
      "Later instructions can depend on state changes from earlier ones",
      "Order affects compute metering but not resulting account state",
      "Order matters only for CPI privilege propagation, not state transitions",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "Derive it from seeds even if the caller did not pass it in the instruction",
      "Receive it from the caller as an account input",
      "Fetch it from RPC during execution based on its pubkey",
      "Load it from an implicit runtime cache keyed by owner program",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "So every validator charges the same fees and priority costs",
      "So all validators converge on the same resulting state",
      "So leaders can reorder conflicting writes without changing outcomes",
      "So lock sets can be inferred after execution instead of before",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "Because each writable account accepts only one signature per slot",
      "Because it would cause write-write conflicts",
      "Because PoH serializes all account writes globally by hash order",
      "Because stake-weighted QoS serializes writes by fee payer class",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "They do not support versioned transaction messages or ALT lookups",
      "They require lookup-table addresses to appear as signer keys",
      "They require the fee payer key to be resolved via lookup tables",
      "They reject compute-budget instructions inside versioned messages",
    ],
    answerIndex: 0,
  ...
  },
  {
    ...
    difficulty: "foundation",
    prompt: "Which statement about instruction data is true for all programs?",
    choices: [
      "The runtime enforces one canonical serialization format before dispatch",
      "Arbitrary bytes interpreted by the program",
      "Data layout is derived from AccountMeta flags during execution",
      "The runtime prepends a universal opcode field before program entry",
    ],
    answerIndex: 1,
    explanation:
  ...
  },
  {
    ...
    choices: [
      "Reconstructing canonical state by replaying emitted log strings",
      "Debugging and UX hints",
      "Submitting vote lockouts to consensus from client apps",
      "Recording fee transfers that replace account lamport deltas",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "It changes blockhash context used for earlier signature verification",
      "It increases compute and account requirements",
      "It resets remaining compute units after the inner invoke returns",
      "It invalidates prior account locks and recomputes lock sets mid-transaction",
    ],
    answerIndex: 1,
  ...
  },
  {
    ...
    choices: [
      "Programs accept passed accounts without validating expected addresses or owners",
      "Programs include duplicate writable metas that point to the same pubkey",
      "Programs reuse one PDA across users to reduce account creation cost",
      "Programs skip data-length checks before deserialization",
    ],
    answerIndex: 0,
  ...
  },
  {
    ...
    choices: [
      "To ensure rent-exempt lamports were deposited before parsing",
      "To avoid panics and ensure the data matches the expected layout",
      "To satisfy BPF alignment rules required for every deserialize call",
      "To prove the account is owned by the expected program",
    ],
    answerIndex: 1,
